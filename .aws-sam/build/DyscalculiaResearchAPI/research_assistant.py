...
class DyscalculiaResearchAssistant:
    ...
    def get_real_studies(self, intervention_name):
        ...
        return all_results[:5]  # Limit to 5 results
    
    def search_intervention(self, intervention_name):
        """Search for research on specific dyscalculia intervention"""
        eric_query = f"dyscalculia AND {intervention_name}"
        pubmed_query = f"dyscalculia[Title/Abstract] AND {intervention_name}[Title/Abstract]"
        scholar_query = f"dyscalculia {intervention_name}"
        
        print(f"Searching for: {intervention_name}")
        print("=" * 50)
        
        eric_results = self.search_eric(eric_query, 3)
        pubmed_results = self.search_pubmed(pubmed_query, 3)
        scholar_results = self.search_google_scholar(scholar_query, 4)
        
        all_results = eric_results + pubmed_results + scholar_results
        
        return self._generate_evidence_brief(intervention_name, all_results)
    
    def _generate_evidence_brief(self, intervention, results):
        """Generate a research brief for the intervention"""
        brief = f"""
EVIDENCE BRIEF: {intervention.upper()} FOR DYSCALCULIA
Generated: {datetime.now().strftime('%Y-%m-%d')}

RESEARCH SUMMARY:
Found {len(results)} relevant studies across ERIC, PubMed, and Google Scholar databases.
...
- Review full studies for detailed methodology
- Consider student age/grade level when applying
- Monitor progress with consistent assessment
- Adapt techniques based on individual needs
...
if __name__ == "__main__":
    ...
    # Search for evidence on number lines
    brief = assistant.search_intervention("number line")
    print(brief)